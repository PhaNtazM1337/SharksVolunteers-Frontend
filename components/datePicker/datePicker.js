    ...
    methods: {
      ...
      _gotDateTimeList({ _start, _end, _type }) {
        ...
        for (let i = _start; i <= _end; i++) {
          ...
          // }
          // else{
          //   resultDataList.push(this._addZ(i) + this.data.dateDetails[_type])
          // }
          resultDataList.push(this._addZore(i) + this.data.dateDetails[_type])
        }
      ...
      _addZ(_num) {
        return _num < 10 ? _num : _num.toString()
      },
      //回显时间
      ...
      selectChangeFn(ev) {
        ...
        selectValues.map((el, index) => {
          ...
          if (index == 0) {
            dateTime += '-'
          } else if (index == 2 || (index == 3 && index != selectValues.length - 1)) {
            dateTime += ':'
          } else if (index == 1 && index != selectValues.length - 1) {
            dateTime += ' '
          }
        })
        // 触发父组件把值传递给父组件
